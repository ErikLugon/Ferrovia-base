checaOLog = ['Checa o log, primata!', 'Checa o log, galinha depenada',
             'Yo phone lingin, cmon big boy, pick yo phon',
             'Checa o log, sabonete de mecânico',
             'Vem dormir comigo pedro, vem dormir comigo',
             'tem alguém me mandando mensagem, checa o log',
             ]

poll = ['A pergunta que não quer calar: \n',
        'Responda aí nego \n',
        'Pergunta séria: \n',
        'Será?',
        'Nada aconteceu na praça da paz celestial \n',
        'Responda ou sofra a punição divina \n',
        'Responda ou morra \n',
]

musica = ['Tocando uma pra você \n',
          'É pra já meu rei... \n',
          'É pra já meu aristocrata... \n',
          'Bongo faz isso?? \n',
          'Pancake faz isso?? \n',
          'Um dia eu vou caçar todos vocês \n',
          'Essa é deliciosa \n',
          'Bota um leno brega pra tocar nessa porra \n',
          'E aí moreno, o que vamos ouvir hoje?',
          'WRRRREEEEEEEEEEEEEEEEEEEEEEEEE'
          ]

xingamentos = ['Vai tomar no cu leandro', 
                   'Odeio judeus',
                   'Desista dos seus sonhos e morra',
                   'Me atire aos lobos, e voltarei morto né porrakkkkkkkkkkkkkkkkkkkkkk',
                   'mimm cago nas kalssah',
                   'Você prefere R$1 ou R$2?',
                   
                   ### Memes do servidor
                   'O tempo não é real',
                   'Já correu com os lobos hoje?',
                   'Corra com os lobos!',
                   'Pneumoultramicroscopicsilicovolcanoconiosis',
                   'Eu sou um herói',
                   'Albion Online é um MMORPG SandBox em que você escreve sua própria história, ao invés de seguir um caminho pré-determinado. Explore um vasto mundo aberto que consiste de 5 ecossistemas únicos. Tudo que você faz gera um impacto no mundo, já que em Albion a economia é produzida pelo jogador. Cada peça de equipamento é construída por jogadores a partir de recursos obtidos por eles. O equipamento que você usa define quem você é, Ir de cavaleiro para feiticeiro é tão fácil quanto trocar a armadura e a arma ou uma combinação das duas. Aventure-se no mundo aberto e enfrente os habitantes e criaturas de Albion. Saia em expedições ou entre em masmorras para enfrentar inimigos ainda mais desafiadores. Enfrente outros jogadores em confrontos do mundo aberto, Lute pelo controle de territórios ou cidade inteiras em batalhas táticas em grupo. Relaxe descansando em sua ilha pessoal, onde você pode construir sua casa, cultivar alimentos e criar animais. Junte-se a uma guilda, tudo fica mais divertido quando se trabalha em equipe. Entre hoje mesmo no mundo de Albion e escreva sua própria história.',
                   'Bora sobe os móveis aí fi kkkkkkkk',
                   'Mas minha rua ta toda manilhada',
                   'Lêmure 78!',
                   'Ela não é pequena nem grande',
                   'Pingou o kingdom hearts lá? kkkkkkk',
                   'Pão',
                   'To ligado que deram !leandro aí, mas acho que o kingdom hearts não pingou ainda',
                   "Baldur's Gate 3 ta 12 conto na gog :eyes:",
                   'uff :fire:',
                   'Rosa eu dou pra quem tá morta pra quem tá vivo eu dou piroca',
                   'nunca vou sentir a dor de um coração partido :fire::fire::fire: até porque eu não tenho coração né porra kkkkkkkkk',
                   'Cê vai na carne ou na mandioca?',
                   'Eles atinjiram a segunda call!',
                   'Os homens do mar atacaram de novo',
                   
                   ### Desgraça
                   'Toma a kunai pra tu ver',
                   'Bora penera a brita',
                   'Qual o preço do cachorro quente, galego?',
                   'Tem como peneirar isso não desgraça?',
                   'Calma pai!',
                   'Chora que eu te dou o pó de 10',
                   '"O ignorante afirma, o sábio duvida, o sensato reflete". - Molestóteles.'
                   'Já sabe onde o sábio é geralmente encontrado né kkkkkkkk',
                   'Não clique na tela durante o loading!',
                   
                
                   
                   
                   ### Memes random
                   'https://www.youtube.com/shorts/TA7mEE2G6e8',
                   'É perigoso ir sozinho, leve isso \n https://www.youtube.com/watch?v=GH9VPYvliJE&list=PLSgQ9WKYqJTUa7XQjn7UQS1OsT7IxYwJE',
                   
                   ### Nerdice
                   'O que é um homem? uma miserável pilha de segredos',
                   'A Lie will remain a Lie!',
                   'Rocha e pedra!',
                   '_mas ninguém veio_',
                   'Oda é burro',                                                      
                ]